...
Fixes mypy type errors and eliminates `Any` usage in the instagram-archiver project.
...
1. Fix all mypy errors reported by `uv run mypy instagram_archiver`
...
Place new TypedDict classes in `instagram_archiver/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`instagram_archiver/typing.py` when shared.
...
1. Run `uv run mypy instagram_archiver` and capture all errors.
...
4. Run `uv run mypy instagram_archiver` again to verify fixes.
