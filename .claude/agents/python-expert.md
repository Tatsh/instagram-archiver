---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the instagram-archiver project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the instagram-archiver project.
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'instagram_archiver': {}})
  ```
...
- `` :py:mod:`instagram_archiver.string` `` for modules
- `` :py:func:`instagram_archiver.string.slugify` `` for functions
- `` :py:class:`instagram_archiver.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~instagram_archiver.string.slugify` `` renders as `slugify`
