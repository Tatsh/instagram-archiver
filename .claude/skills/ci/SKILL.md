...
If any changed files are under `instagram_archiver/` or `tests/`, run the following
agents **in order**:
...
1. **click-auditor** - validate Click command consistency. **Only run if files under
   `instagram_archiver/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `instagram_archiver/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `instagram_archiver/commands/main.py`,
...
For Python files, strip the `instagram_archiver/` prefix and replace `/` with `.` (like module imports).

- Python file `instagram_archiver/media.py` → `media:`.
- Multiple files under `instagram_archiver/commands/` → `commands:`.
- Single command file `instagram_archiver/commands/admin.py` → `commands.admin:`.
